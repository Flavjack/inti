...
`factors_crd_3f`` ``<-`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
...
`crd_exp_3f`` ``<-`` `[`design_repblock`](https://inkaverse.com/reference/design_repblock.md)`(`\
`  nfactors ``=`` ``3``,`\
...
\
`# Fieldbook preview`\
`crd_exp_3f``$``fieldbook`` `[`%>%`](https://magrittr.tidyverse.org/reference/pipe.html)\
