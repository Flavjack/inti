...
url <- "https://docs.google.com/spreadsheets/d/1QziIXGOwb8cl3GaARJq6Ez6aU7vND_UHKJnFcAKx0VI/edit#gid=137089581"
gs <- as_sheets_id(url)
# browseURL(url)
...
fb <- xl %>% 
  ...
  mutate(across(where(is.factor), ~gsub("\\s+|\\.", "_", .) )) %>% 
  as.data.frame()
# str(fb)
```
...
ggt <- git %>% 
  fplot(type = "line", 
          x = "evaluation", xlab = "Day",
          y = "mean", ylab = "Germination ('%')",
          groups = "nacl", glab = "NaCl (Mpa)", 
          limits = c(0,120), brakes = 10,
          color = F)
...
fg <- jpeg::readJPEG("files/fig_jc.jpg") %>% 
...
fig <- include_figure(
  ...
  , caption = "Plant of *Jatropha curcas*. A) Foliage, B) Leaf, C) Fruit."
  )
...
fg_01 <- png::readPNG("files/fig_pca_var.png") %>% 
  grid::rasterGrob(interpolate = TRUE)

fg_02 <- png::readPNG("files/fig_pca_ind.png") %>% 
  grid::rasterGrob(interpolate = TRUE)

fg_03 <- png::readPNG("files/fig_cluster_tree.png") %>% 
  grid::rasterGrob(interpolate = TRUE)

fg_04 <- png::readPNG("files/fig_cluster_map.png") %>% 
  grid::rasterGrob(interpolate = TRUE)
...
fig <- include_figure(
  ...
  , caption = "Multivariate Analysis: Principal component Analysis and Hierarchical Clustering Analysis."
  )
